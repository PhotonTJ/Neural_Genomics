...
*▶ Video walkthrough: Model Collapse as Latent Manifold Flattening*
 ...
 **recursive self-merging**. Here, a model is iteratively merged with its descendants in a chain-like fashion—e.g.,
 $\text{Parent}_1+\text{Child}_1\to\text{Child}_2$, then $\text{Child}_1+\text{Child}_2\to\text{Child}_3$, and so on—
...
*▶ Video walkthrough: Collapse Trajectories and nDNA Diagnostics*
...
Formally, at each generation $g$, the model $M^{(g)}$ is fine-tuned on a dataset $D^{(g)}$ constructed entirely from the outputs of its predecessor:
 D(g) = Output(M(g-1), Alpaca Prompts)
We track the evolution of the model's **latent geometry**—including
 **spectral curvature** $\kappa_{\ell}$,
 **thermodynamic length** $\mathcal{L}_{\ell}$, and
 **belief vector norm** $\lVert\mathbf{v}_\ell^{(c)}\rVert$—to detect indicators of semantic collapse.
...
These plots reveal how repeated merging (each generation combines with its base model) induces collapse, seen as contraction of thermodynamic length
 ($\mathcal{L}_{\ell}$) and flattening of spectral curvature ($\kappa_{\ell}$).
