...
**Spectral curvature ($\kappa$) — turbulent.** *Fluid:* a bend induces
...
**Belief field ($\mathbf{v}$).** *Fluid:* the velocity field sets
...
$$\underbrace{\mathcal{L}_{\text{pretrain}}(\theta) = \mathbb{E}_{(x,y)} [-\log p_\theta(y|x)]}_{\text{SGD as selection pressure}}$$
...
where $\gamma_c$ is the latent trajectory for corpus $c$. This latent geometry echoes Waddington’s epigenetic landscape where paths represent developmental fates [83].
...
| **Context regulator** | Conditional $P(\text{gene ON} \mid \text{TF, epi})$; Bayesian gene networks [68] | Conditional latent path $P(h_1, \ldots, h_\ell \mid x)$; stochastic latent dynamics [71] |
...
[38] Hyv{"a}rinen, Aapo “Estimation of Non-Normalized Statistical Models by Score Matching” *Journal of Machine Learning Research* (2005). [https://jmlr.org/papers/v6/hyvarinen05a.html](https://jmlr.org/papers/v6/hyvarinen05a.html)
